...
> GroundTruth weighs every report against every other report — so you know what's actually happening, even when the internet is down.
...
Standard AI tools need cloud connectivity to work. But crises are precisely when cell towers go down, internet fails, and you're on your own.
...
5. **Outputs calibrated confidence** — not just "flooding detected" but "88% confidence — avoid the Lekki-Epe underpass"

All of this runs on a single device, no internet required.

---

## Why Gemma 4

GroundTruth uses Gemma 4's differentiating features deeply, not superficially:
...
- **Gemma 4 resolves it**: the contradicting report is downweighted because it lacks visual evidence and a 45cm water-level sensor independently confirms the flooding
- **Result**: 93% confidence — avoid the underpass

This is the exact kind of life-or-death decision the system is designed for.
...
```bash
git clone https://github.com/YOUR_USERNAME/groundtruth
cd groundtruth
npm install
...
- Works across iOS Safari PWA + desktop Node.js simultaneously
- Reports from one device appear on all others within seconds

The `npm run dev` script activates the mesh server automatically. Open the app on two devices on the same Wi-Fi and watch the "Local Mesh" panel connect.
...
Once installed, the UI works fully offline. AI inference requires Ollama locally or a Google API key.
...
│   ├── signal-normalizer.ts    # Raw input → structured signal
